...
describe("ExtensionLifecycle", () => {
  ...
  describe("activate", () => {
    ...
    it("should collapse the auxiliary bar when a new editor group is opened and the setting is enabled", async () => {
      vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
        get: vi.fn((key: string, defaultValue?: unknown) => {
          if (key === "enableTmux") return true;
          if (key === "collapseSecondaryBarOnEditorOpen") return true;
          return defaultValue;
        }),
      ...
      vscode.window.tabGroups.all = [{ tabs: [{ id: "tab-1" }] }];

      await lifecycle.activate(mockContext);

      const listener = vi.mocked(vscode.window.tabGroups.onDidChangeTabGroups)
    ...
    it("should not collapse the auxiliary bar when the setting is disabled", async () => {
      vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
        get: vi.fn((key: string, defaultValue?: unknown) => {
          if (key === "enableTmux") return true;
          if (key === "collapseSecondaryBarOnEditorOpen") return false;
          return defaultValue;
        }),
        inspect: vi.fn(() => undefined),
        update: vi.fn(),
